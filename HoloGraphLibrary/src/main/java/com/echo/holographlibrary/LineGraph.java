...
public class LineGraph extends View {
    ...
    public void onDraw(Canvas ca) {
        if (fullImage == null || shouldUpdate) {
            ...
            for (Line line : lines){
                ...
                if (lineCount == lineToFill){
                    ...
                    for (LinePoint p : line.getPoints()){
                        ...
                        if (count == 0){
                            lastXPixels = sidePadding + (xPercent*usableWidth) + leftPadding;
                            lastYPixels = getHeight() - bottomPadding - (usableHeight*yPercent);
                            firstXPixels = lastXPixels;
                            path.moveTo(lastXPixels, lastYPixels);

                            Path pa = new Path();
                            pa.moveTo(0, getHeight());
                            pa.lineTo(leftPadding, getHeight());
                            pa.lineTo(leftPadding, 0);
                            pa.lineTo(0, 0);
                            pa.close();
                            canvas.drawPath(pa, paint);

                        } else {
                            newXPixels = sidePadding + (xPercent*usableWidth) + leftPadding;
            ...
            for(float i=1; i< 5; ++i){
                float barHeight=getHeight()/5*i-bottomPadding;
                canvas.drawLine(sidePadding,barHeight  , getWidth()-sidePadding, barHeight, paint);
                float yPercent = 1-((barHeight)/(getHeight()));
                canvas.drawText(""+(int)(minY+yPercent*(maxY - minY)), 5, barHeight-3, paint);
            }
            canvas.drawText(""+(int)(minY), 5, getHeight()-bottomPadding-3, paint);
            paint.setAlpha(255);
            paint.reset();
